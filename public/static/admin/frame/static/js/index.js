...
layui.use(['layer', 'element', 'util'], function () {
    ...
    window.addTab = function (elem, tit, url) {
        ...
        var id = src;                             // ID
        var flag = getTitleId(card, src);                          // 是否有该选项卡存在
        ...
        if (flag != 'begin') {
            ...
            if (src) {
                //新增
				if (id == "/Cs/Pay/recharge.html") {
					title = "充值管理";
				}
				if (id == "/Cs/Pay/draw.html") {
					title = "提现管理";
				}
                element.tabAdd(card, {
		...
		// 刷新当前页
		var index = 0, cardIdx = 0;
		$(document).find(".my-body .layui-tab-card > .layui-tab-title li").each(function () {
